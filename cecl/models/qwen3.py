...
class KVCache(flax.struct.PyTreeNode):
    ...
    def create(cls, num_layers, batch_size, max_seq_len, head_dim, kv_heads):
        k = [
            jnp.zeros((batch_size, max_seq_len, kv_heads, head_dim), dtype=jnp.bfloat16)
    ...
    def get_sharding(cls, cache_shard, none_shard):
        ...
        return KVCache(
            k=cache_shard, v=cache_shard, length=none_shard, starts=cache_shard
        )
...
class Block(nn.Module):
    ...
    def __call__(self, x, sin, cos, token_mask, layer_id, cache=None):
        ...
        pre_gamma = self.param(
            "pre_gamma", nn.initializers.constant(1.0), (self.hidden_size,)
        )
        x_norm = rms_norm(x, pre_gamma, self.eps)

        # Calculate Q,K,V.
        q = nn.Dense(self.q_heads * self.head_dim, use_bias=False, dtype=jnp.bfloat16)(
            x_norm
        )
        q = jnp.reshape(q, (q.shape[0], q.shape[1], self.q_heads, self.head_dim))
        k = nn.Dense(self.kv_heads * self.head_dim, use_bias=False, dtype=jnp.bfloat16)(
            x_norm
        )
        k = jnp.reshape(k, (k.shape[0], k.shape[1], self.kv_heads, self.head_dim))
        v = nn.Dense(self.kv_heads * self.head_dim, use_bias=False, dtype=jnp.bfloat16)(
            x_norm
        )
        ...
        if cache is not None:
            k = jax.lax.dynamic_update_slice_in_dim(
                cache.k[layer_id], k, cache.length, axis=1
            )
            v = jax.lax.dynamic_update_slice_in_dim(
                cache.v[layer_id], v, cache.length, axis=1
            )
            time_idx = jnp.arange(0, v.shape[1], dtype=jnp.int32)[
                None, :
            ]  # [1, seqlen]
            ...
            k_idx = (time_idx >= cache.starts[:, None]) & (
                time_idx < (cache.length + incremental_pos).astype(jnp.int32)
            )
        ...
        # === MLP Block.
        # =========================
        post_gamma = self.param(
            "post_gamma", nn.initializers.constant(1.0), (self.hidden_size,)
        )
        x_norm = rms_norm(x, post_gamma, self.eps)
        g = nn.Dense(features=self.mlp_ffw_size, use_bias=False, dtype=jnp.bfloat16)(
            x_norm
        )
        g = nn.silu(g)
        y = nn.Dense(features=self.mlp_ffw_size, use_bias=False, dtype=jnp.bfloat16)(
            x_norm
        )
        y = g * y
        mlp_x = nn.Dense(features=self.hidden_size, use_bias=False, dtype=jnp.bfloat16)(
            y
        )
        x = x + mlp_x
        return x, k, v
    ...
    def __call__(self, x, token_mask, cache=None, get_logits=True):
        ...
        for layer_id in range(self.num_layers):
            x, k, v = Block(
                ...
                eps=self.eps,
            )(x, sin, cos, token_mask, layer_id, cache)
            if cache is not None:
                cache.k[layer_id] = k
                cache.v[layer_id] = v

        gamma_final = self.param(
            "gamma_final", nn.initializers.constant(1.0), (self.hidden_size,)
        )
        ...
        if cache is not None:
            cache = cache.replace(
                length=cache.length + jnp.max(length_minus_padding(token_mask))
            )
